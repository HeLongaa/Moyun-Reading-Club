  ...
  <div class="login-bg">
    <div class="login-container">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          <label>新密码</label>
          <input v-model="password" type="password" required placeholder="请输入新密码">
        </div>
        <div class="form-group">
          <label>确认新密码</label>
          <input v-model="password2" type="password" required placeholder="请再次输入新密码">
        </div>
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      this.success = ''
      try {
        await authApi.resetPassword({ email: this.email, password: this.password })
        this.success = '密码重置成功，请返回登录！'
      } catch (e) {
        this.error = e.response?.data?.message || '重置失败，请检查邮箱是否正确'
      } finally {
        this.loading = false
      }
    }
